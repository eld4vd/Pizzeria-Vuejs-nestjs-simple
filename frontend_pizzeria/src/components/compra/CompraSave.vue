...
// Calcular fecha máxima (hoy + 1 día por zona horaria)
...
// Calcular fecha mínima (1 año atrás)
const fechaMinima = computed(() => {
  const oneYearAgo = new Date()
  oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1)
  return oneYearAgo.toISOString().substring(0, 16)
})
...
const validarFecha = (): boolean => {
  ...
  if (!form.value.fechaCompra) {
    return true // Se usará la fecha actual en el backend
  }

  const fecha = new Date(form.value.fechaCompra)
  ...
  if (isNaN(fecha.getTime())) {
    errorFecha.value = 'La fecha no es válida'
    return false
  }

  if (fecha > manana) {
    errorFecha.value = 'La fecha de compra no puede ser una fecha futura'
    return false
  }

  if (fecha < unAnoAtras) {
    errorFecha.value = 'La fecha de compra no puede ser mayor a 1 año de antigüedad'
    return false
  }
...
const guardar = async () => {
  ...
  if (!validarFecha()) {
    toast.warning(errorFecha.value)
    return
  }
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <input
                    ...
                    required
                  />
                  <div v-if="errorFecha" class="invalid-feedback" style="display: block;">
                    {{ errorFecha }}
                  </div>
                  <small class="text-muted">
                    <i class="fas fa-info-circle mr-1"></i>
                    La fecha no puede ser futura ni mayor a 1 año
                  </small>
                </div>
              </div>
            </div>
