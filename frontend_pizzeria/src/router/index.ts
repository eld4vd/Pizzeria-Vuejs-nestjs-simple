...
const router = createRouter({
  ...
  routes: [
    ...
    },
    {
      path: '/register',
      name: 'register',
      component: () => import('../views/RegisterClienteView.vue'),
    },
    {
      path: '/dashboard',
...
router.beforeEach(async (to) => {
  const publicPages = ['/login', '/register', '/', '/menu', '/about', '/services', '/blog', '/contact', '/single-blog', '/elements', '/mis-pedidos']
  ...
  // Si requiere ser admin/empleado y es un cliente
  if (to.meta.requiresAdmin && authStore.isAuthenticated) {
    const rol = authStore.userRole
    if (rol === 'cliente') {
      // Los clientes NO pueden acceder al dashboard
      return '/menu'
    }
  }
})
